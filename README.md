...
The range of an `n`-bit signed integer can be calculated with the following formula:

\[
-\left(2^{(n-1)}\right) \text{ to } 2^{(n-1)} - 1
\]

### Applying the Formula for a 32-bit Integer

For the `int` data type in Java:
- `n = 32` (since it's a 32-bit integer)
- Using the formula, the range becomes:
...
Therefore, the range of a 32-bit signed integer (`int`) in Java is from **-2147483648** to **2147483647**. This range ensures that the integer type can represent both positive and negative numbers, with one bit dedicated to indicating the sign.
...
This range is a fundamental characteristic of the `int` type in Java, important to consider when working with integer values.
